...
std::vector<Point> Torus::draw() const {
    ...
    float ang_stack = 2 * M_PI / _stacks;
    ...
    for(i32 slice = 0; slice < _slices; slice++) {
        ...
        {
            //VectorSpherical   border = VectorSpherical(_ring_radius, ang_stack * stack      ,   center.azimuth());
            //VectorSpherical n_border = VectorSpherical(_ring_radius, ang_stack * (stack + 1), n_center.azimuth() );
            auto border = Vector(0, 1, 0);
            auto n_border = Vector(0, 1, 0);
