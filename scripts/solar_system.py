...
Mercury = Astro('Mercury', 0.1, 6 , 1, 0, 0, [])
Venus   = Astro('Venus'  , 0.1, 7 , 1, 0, 0, [])
Earth   = Astro('Earth'  , 0.1, 8 , 1, 0, 0, generate_moons('Earth', 5, 10))
Mars    = Astro('Mars'   , 0.1, 9 , 1, 0, 0, generate_moons('Mars', 5, 10))
Jupiter = Astro('Jupiter', 0.1, 11, 1, 0, 0, generate_moons('Jupiter', 5, 10))
Saturn  = Astro('Saturn' , 0.1, 12, 1, 0, 0, generate_moons('Saturn', 5, 10))
Uranus  = Astro('Uranus' , 0.1, 13, 1, 0, 0, generate_moons('Uranus', 5, 10))
Neptune = Astro('Neptune', 0.1, 14, 1, 0, 0, generate_moons('Neptune', 5, 10))
Pluto   = Astro('Pluto'  , 0.1, 15, 1, 0, 0, generate_moons('Pluto', 5, 10))
